...
const shadowPrepareStarted = ref(false)
const rawSimulationId = computed(() => {
  const v = route.query?.rawSimulationId
  return typeof v === 'string' ? v.trim() : ''
})
const disambSimulationId = computed(() => {
  const v = route.query?.disambSimulationId
  return typeof v === 'string' ? v.trim() : ''
})
const shadowSimulationId = computed(() => {
  if (props.simulationId && rawSimulationId.value && disambSimulationId.value) {
    return props.simulationId === rawSimulationId.value
      ? disambSimulationId.value
      : rawSimulationId.value
  }
  const shadow = route.query?.shadowSimulationId
  return typeof shadow === 'string' ? shadow.trim() : ''
})
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // AB 模式：并行触发另一套 simulation 的 prepare（后台跑，不阻塞当前页面）
      startShadowPrepare()
    } else {
...
const startShadowPrepare = async () => {
  if (!shadowSimulationId.value || shadowPrepareStarted.value) return
  shadowPrepareStarted.value = true
  try {
    const res = await prepareSimulation({
      simulation_id: shadowSimulationId.value,
      use_llm_for_profiles: true,
      parallel_profile_count: 5
    })
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog(`AB并行：另一套 simulation 已准备完成（${shadowSimulationId.value}）`)
      } else {
        addLog(`AB并行：已启动另一套 prepare 任务（${shadowSimulationId.value}）`)
      }
    } else {
      addLog(`AB并行：另一套 prepare 启动失败（${res.error || 'unknown'}）`)
    }
  } catch (err) {
    addLog(`AB并行：另一套 prepare 异常（${err.message}）`)
  }
}
